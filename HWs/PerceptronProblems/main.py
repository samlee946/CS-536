...
def perceptron(X, Y, max_epoch = 1000, realtime_plot = False):
    m, k = X.shape
    w = np.zeros((k, 1))
    X[:, 0] = np.ones(m)
    steps = 0
    w_history = np.zeros((max_epoch, 3))
    w_history_2 = []
    act_epoch = 0
    epoch = 0
    while epoch < max_epoch:
        ...
        for i in range(m):
            ...
            if f != Y[i]:
                ...
                #print(w)
                w_history[epoch] = w.reshape(3)
                w_history_2.append(w)
        act_epoch = max(act_epoch, epoch)
        #print(err_cnt)
        max_epoch = min(max_epoch, epoch + 2 * err_cnt)
        epoch += 1
        if flag == False:
            break
    if realtime_plot == True:
        ...
        plt.xlabel('epoch')
        plt.ylabel('value')
        line_b, = ax.plot(w_history[:, 0], '-b', label='b')
        line_w1, = ax.plot(w_history[:, 1], '-r', label='w_1')
        line_w2, = ax.plot(w_history[:, 2], '-k', label='w_2')
...
def question5(save = False, save_file = 'q5.png'):
    ...
    w, steps = perceptron(X, Y, max_epoch = 500, realtime_plot = True)
    #fig, ax = plt.subplots()
    #plt.xlabel('x_1')
    #plt.ylabel('x_2')
    #idx_neg = []
